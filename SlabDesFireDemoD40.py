...
class SlabDesFireDemoD40(unittest.TestCase):
    ...
    def _clearCard(self):
        if SlabDesFireDemoD40.enable_vc:
            self._send_nocheck(cmd.vcISOSelect(SlabDesFireDemoD40.desfire_aid,
                                               SlabDesFireDemoD40.key128_default, SlabDesFireDemoD40.key128_default))
            self._send_nocheck(cmd.vcProximityCheck(SlabDesFireDemoD40.key128_default))

        else:
            self._send(cmd.SelectApp(0))
        self._send(cmd.AuthenticateISO(0, SlabDesFireDemoD40.key64_default))
    ...
    def test_02(self):
        ...
        * This demo is to create applications, change key(s) and authenticate 
        ...
        self.log_op('Authenticate application 1 master key in D40 DES_64')._send(
            cmd.Authenticate(0, SlabDesFireDemoD40.key64_default))
        self.log_op('Change application 1 key 1 to new key, DES_64')._send(cmd.ChangeKey(
            1, SlabDesFireDemoD40.nk64C, SlabDesFireDemoD40.key128_default, None))
        self.log_op('Change application 1 key 2 to new key, DES_128')._send(cmd.ChangeKey(
            2, SlabDesFireDemoD40.nk128, SlabDesFireDemoD40.key128_default, None))

        self.log_op('Authenticate application 1 key 1 in D40 DES_64')
        self._send(cmd.Authenticate(1, SlabDesFireDemoD40.nk64))

        self.log_op('Get card UUID')._send(cmd.GetCardUID())

        self.log_op('Authenticate application 1 key 2 in D40 DES_128')
        self._send(cmd.Authenticate(2, SlabDesFireDemoD40.nk128))
    ...
    def test_03(self):
        ...
        * This demo is to show how to create , operation and delete standard/backup/record
        ...
        self.log_op(
            'Create application DES64/128')
        self._send(cmd.CreateApp(0x01, 0x0B, 0x0E))

        #####################################################################################
        self.log_op('Select application 1')._send(cmd.SelectApp(1))

        self.log_op('Authenticate application 1 D40 DES_64')
        self._send(cmd.Authenticate(0, SlabDesFireDemoD40.key64_default))

        self.log_op('Change application 1 key 1 to new key DES_128')
        self._send(cmd.ChangeKey(1, SlabDesFireDemoD40.nk128_1, SlabDesFireDemoD40.key128_default, None))

        self.log_op('Change application 1 key 2 to new key DES_128')
        self._send(cmd.ChangeKey(2, SlabDesFireDemoD40.nk128_2, SlabDesFireDemoD40.key128_default, None))

        self.log_op('Change application 1 key 3 to new key DES_128')
        self._send(cmd.ChangeKey(3, SlabDesFireDemoD40.nk128_3, SlabDesFireDemoD40.key128_default, None))

        self.log_op('Change application 1 key 4 to new key DES_128')
        self._send(cmd.ChangeKey(4, SlabDesFireDemoD40.nk128_4, SlabDesFireDemoD40.key128_default, None))
        ...
        self._send(cmd.Authenticate(3, SlabDesFireDemoD40.nk128_3))
        ###############################################################################
        # File 1
        ...
        SlabDesFireDemoD40.data16[15] = 0x40
        self._send(cmd.WriteRecord(4, 0, 16, 'plain', SlabDesFireDemoD40.data16))
        self.log_op('Commit transaction')
        self._send(cmd.CommitTransaction(False))

        SlabDesFireDemoD40.data16[15] += 1
        self._send(cmd.WriteRecord(4, 0, 16, 'plain', SlabDesFireDemoD40.data16))
        self.log_op('Commit transaction')
        self._send(cmd.CommitTransaction(False))
        ...
        SlabDesFireDemoD40.data16[15] = 0x50
        self._send(cmd.WriteRecord(5, 0, 16, 'full', SlabDesFireDemoD40.data16))
        self.log_op('Commit transaction')
        self._send(cmd.CommitTransaction(False))
        ...
        ###############################################################################
        self.log_op('Authenticate application 1 key 4 D40 DES_128')
        self._send(cmd.Authenticate(4, SlabDesFireDemoD40.nk128_4))
        ...
        self._send(cmd.GetFileSettings(1))
        ###############################################################################
        self.log_op('Authenticate application 1 master key D40 DES_64')
        self._send(cmd.Authenticate(0, SlabDesFireDemoD40.key64_default))
    ...
    def stest_04(self):
        ...
        * This demo is to show how to create , operation and delete value files,
        ...
        ''')
        self._clearCard()
        self.log_op('Create application DES64/128')
        self._send(cmd.CreateApp(0x01, 0x0B, 0x0E))
        ...
        self.log_op('Authenticate key 3 for file operation')
        self._send(cmd.Authenticate(3, SlabDesFireDemoD40.nk128_3))
        ###############################################################################
        # File 3
        ...
        self._send(cmd.GetValue(3, 'mac'))
        ###############################################################################
        self.log_op('Authenticate application 1 master key in D40 DES_64')
        self._send(cmd.Authenticate(0, SlabDesFireDemoD40.key64_default))
        self.log_op('Delete all files')
        self.log_op('Delete')
        for x in (3,):
